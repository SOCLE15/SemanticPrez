...
class VideoService {
	...
	static private $services = [
		...
		'asciinema' => [
			'embed'			=> '<script type="text/javascript" src="https://asciinema.org/a/%1$d.js" id="asciicast-%1$d" async></script>',
			'default_width'	=> 640,
			'default_ratio'	=> 1.59609120521173, //(490 / 307)
			'https_enabled'	=> true,
			'url_regex'		=> [
				'#asciinema\.org/a/([\d]+)#is'
			],
			'id_regex'		=> [
				'#^([\d]+)$#is'
			]
		...
		'slideshare' => [
			'embed'			=> '<iframe src="//fr.slideshare.net/slideshow/embed_code/key/%1$s" width="%2$d" height="%3$d" frameborder="0" allowfullscreen="true"></iframe>',
			'default_width' => 425,
			'default_ratio' => 1,1971831, //(425 / 355)
			'https_enabled'=> true,
			'url_regex'		=> [
				'#fr\.slideshare\.net/slideshow/embed_code/key/([a-zA-Z0-9]+)#is'
			],
			'id_regex'		=> [
				'#([a-zA-Z0-9]+)$#is'
			]
